...
# Oteller ve Tarihler (Aynı liste)
HOTELS = {
...
def main():
    ...
    # Yeni verileri eskisinin ÜSTÜNE yazıyoruz (Sadece son durum kalsın diye)
    df.to_csv('son_durum.csv', index=False)
    
    # Ekstra: Eğer BOŞ ODA bulunduysa, onları 'bulunan_odalar_gecmisi.csv' dosyasına ekle
    bos_odalar = df[df['Durum'].str.contains("✅")]
    if not bos_odalar.empty:
        # Eski geçmiş dosyası varsa oku, üstüne ekle
        try:
    ...
    print("Tarama tamamlandı ve CSV dosyalarına kaydedildi.")
